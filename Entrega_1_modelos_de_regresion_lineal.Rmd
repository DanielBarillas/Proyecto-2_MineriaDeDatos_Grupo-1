...
Vamos a eliminar los valores atipicos
```{r}
...
hatvalues <- hatvalues(modelo1)

# Definir umbrales para identificar valores atípicos influyentes
umbral_cook <- 4 / nrow(train)  # Umbral típico para Cook’s Distance
umbral_leverage <- 2 * mean(hatvalues)  # Umbral para leverage

# Identificar índices de valores atípicos
outliers_cook <- which(cooksd > umbral_cook)
outliers_leverage <- which(hatvalues > umbral_leverage)

# Unir ambos conjuntos de valores atípicos
outliers_influyentes <- unique(c(outliers_cook, outliers_leverage))

# Eliminar los valores atípicos influyentes
train_sin_atipicos <- train[-outliers_influyentes, ]

# Ajustar un nuevo modelo sin estos valores influyentes
modelo_sin_atipicos <- lm(SalePrice ~ ., data = train_sin_atipicos)



# Identificar y eliminar solo el outlier extremo
outlier_adicional <- 81  
train_sin_outlier_adicional <- train_sin_atipicos[-outlier_adicional, ]

# Reajustar el modelo sin este punto
modelo_final <- lm(SalePrice ~ ., data = train_sin_outlier_adicional)

# Volver a graficar los residuos
par(mfrow = c(1, 2))
plot(modelo_sin_atipicos, which = 1, main = "Antes de eliminar el outlier 81")
plot(modelo_final, which = 1, main = "Después de eliminar el outlier 81")
par(mfrow = c(1, 1))
...
modelo_log <- lm(SalePrice ~ ., data = train)

# Graficar residuos para verificar mejoras
par(mfrow = c(1, 1))
plot(modelo_log, which = 1, main = "Residuos después de log(SalePrice)")
...
lillie.test(modelo_log$residuals)
```
...
- Lo primero que vamos a hacer es utilizar VIF o variance inflation factor , que mide las varianzas entre las variables independientes. 
- Un VIF > 10 indica alta multicolinealidad y la variable debería ser eliminada o transformada.
- Un VIF entre 5 y 10 sugiere moderada multicolinealidad, por lo que se recomienda revisar.
- Un VIF < 5 es aceptable.
